/**
 * SAP Courses App — Demo Video Recorder v3
 *
 * Clean rebuild:
 *   ✓ No duplicate scenes or subtitles
 *   ✓ Subtitle appears FIRST so viewer knows what's about to happen
 *   ✓ Highlight (red ring + arrow) → action → result — all visible
 *   ✓ Generous timing so nothing feels rushed
 *   ✓ 16 scenes, ~3 min video
 *
 * Prerequisites:
 *   1. Chrome launched with --remote-debugging-port=9222
 *   2. Logged in to SAP — Fiori Launchpad fully loaded
 *   3. Run: node record_demo_v3.js
 */
...
const FRAMES_DIR = path.join(OUT_DIR, 'frames_v3');
const SS_DIR     = path.join(OUT_DIR, 'screenshots_v3');
const VIDEO_FILE = path.join(OUT_DIR, 'SAP_Courses_Demo_v3.mp4');
const FPS = 10;
let frameIndex = 0;
...
const SCENES = [
    ...
    /* ─── 04: Open Analytics Dashboard ────────────────────────── */
    {
        ...
        action: async (page) => {
            ...
            await page.waitForTimeout(3000); // wait for dialog to open and render
        }
    },
    ...
    /* ─── 10: Navigate to My Assignments ──────────────────────── */
    {
        id:       '10_my_assignments',
        ...
        action: async (page) => {
            ...
            await page.waitForTimeout(4000); // wait for navigation and render
        }
    },

    /* ─── 11: Assignments Overview ─────────────────────────────── */
    {
        id:       '11_assignments_overview',
        subtitle: 'My Assignments: KPI progress cards and training list with Status, Due Date, and completion tracking.',
        readMs:   2000,
        holdMs:   4500,
        action: async (page) => {
            await page.evaluate(() => window.scrollTo(0, 0));
    ...
    /* ─── 12: Assign Training to User ──────────────────────────── */
    {
        id:       '12_assign_training',
...
(async () => {
    ...
    console.log(`\nStarting v3 recording — ${SCENES.length} scenes, ~${Math.round(totalDuration/1000)}s\n`);
